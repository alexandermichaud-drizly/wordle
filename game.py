...
class Game:
    ...
    def submit_guess(self, guess: str):
        ...
        if game_won or self.guesses == 6:
            self.__save_game()
        return [self.CORRECT_LETTER] * 5 if game_won else self.__evaluate_guess(guess)
    ...
    def __evaluate_guess(self, guess: str) -> list:
        ...
        for i, letter in enumerate(guess):
            if ans[i] == letter:
                result[i] = self.CORRECT_LETTER
                guess = guess[:i] + '_' + guess[i+1:]
                ans = ans[:i] + '_' + ans[i+1:]

        for i, letter in enumerate(guess):
            if letter != '_' and letter in ans:
                result[i] = self.INCORRECT_PLACEMENT 
                guess = guess[:i] + '_' + guess[i+1:]
                ans[ans.find(letter)] = '_'
                
        if not all(result):
            return list(map(lambda x: self.INCORRECT_LETTER if x is None else x, result))
